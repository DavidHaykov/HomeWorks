...
import telran.cars.view.ConsoleInputOutput;
...
public class AddModelItem extends RentCompanyItem {
    ...
    private static final int MAX_TANK = 80;
    ...
    public void perform() throws IOException {
        ...
        Integer gasTank = inOut.inputInteger(String.format(
                "Enter gas tank volume from range [%d-%d]", MIN_TANK, MAX_TANK
        ), MIN_TANK, MAX_TANK);
        if(gasTank == null){
            return;
        }
        ...
        Integer priceDay = inOut.inputInteger(String.format("Enter price per day from range [%d-%d]", MIN_PRICE, MAX_PRICE), MIN_PRICE, MAX_PRICE);
        if(priceDay == null){
            return;
        }
